...
parser.add_argument('--lr', type=float, default=0.00000002, help='Learning Rate. Default=0.02')   # 0.00000002
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 1):
        ...
        for d in range(3):
            output_masked[:,d,:,:] = torch.reshape(input_mask[:,d,:,:].unsqueeze(1) * output,(input.shape[0],160,160))
        if cuda:
            output_masked = output_masked.cuda()
        result = NetC(output_masked)
        target_masked = input.clone()
        for d in range(3):
            target_masked[:,d,:,:] = torch.reshape(input_mask[:,d,:,:].unsqueeze(1) * target,(input.shape[0],160,160))
        if cuda:
    ...
    print("===> Epoch[" + str(epoch)+"]("+ str(i)+ "/" + str(len(dataloader))+ ": Batch Dice: {:" + str(1 - loss_dice.item()))
    print("===> Epoch[" + str(epoch)+"]("+ str(i)+ "/" + str(len(dataloader))+ ": G_Loss: {:" + str(loss_G.item()))
    print("===> Epoch[" + str(epoch)+"]("+ str(i)+ "/" + str(len(dataloader))+ ": D_Loss: {:" + str(loss_D.item()))
    ...
    if epoch % 10 == 0:
        ...
        if mIoU > max_iou:
            max_iou = mIoU
            torch.save(NetS.state_dict(), 'outputs/NetS_epoch_' + str(epoch) + '.pth')
        vutils.save_image(data[0],
                'outputs/input_val.png',
                normalize=True)
        vutils.save_image(data[1],
                'outputs/label_val.png',
                normalize=True)
        pred = pred.type(torch.FloatTensor)
        vutils.save_image(pred.data,
                'outputs//result_val.png',
                normalize=True)
    if epoch % 25 == 0:
        lr = lr*decay
        if lr <= 0.00000001:
