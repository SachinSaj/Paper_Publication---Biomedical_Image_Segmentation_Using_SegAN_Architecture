...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root):
        ...
        #sort file names
        print(sorted(glob(os.path.join(self.root, '{}/*.jpg'.format("Pro_imges/Scans/HGG")))))
        self.input_paths = sorted(glob(os.path.join(self.root, '{}/*.jpg'.format("Train/Scan"))))
        self.label_paths = sorted(glob(os.path.join(self.root, '{}/*.jpg'.format("Train/Label"))))
        self.name = os.path.basename(root)
        if len(self.input_paths) == 0 or len(self.label_paths) == 0:
        
            raise Exception("HGG-No images/labels are found in {}".format(self.root))
...
class Dataset_val(torch.utils.data.Dataset):
    def __init__(self, root):
        ...
        #sort file names
        self.input_paths = sorted(glob(os.path.join(self.root, '{}/*.jpg'.format("Val/Scan"))))
        self.label_paths = sorted(glob(os.path.join(self.root, '{}/*.jpg'.format("Val/Label"))))
        self.name = os.path.basename(root)
        if len(self.input_paths) == 0 or len(self.label_paths) == 0:
            raise Exception("LGG-No images/labels are found in {}".format(self.root))
